% Order 1 explicit Symplectic Euler 1 method
% For separable Hamiltonians only!

function [T, Y] = method_se1(f, t, ci)
    % This function performs the explicit Symplectic Euler 1 method (a first-order
    ...
    for i = 1:m-1
        dy = f(t(i), y(:, i));                       % Compute the derivative (slope) at the current point
        y(n+1:2*n, i+1) = y(n+1:2*n, i) + h * dy(n+1:2*n); % Update the second half of the state vector
        dy = f(t(i+1), y(:, i+1));                   % Compute the derivative at the next time point
