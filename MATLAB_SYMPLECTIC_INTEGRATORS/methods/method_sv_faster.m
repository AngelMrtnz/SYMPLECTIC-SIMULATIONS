...
function [T, Y] = method_sv_faster(f1, f2, t, ci)
    ...
    for i = 1:m-1
        ...
        dy = f2(t(i), y(:, i));                 % Compute the derivative (momentum) at the current time
        pm = y(n+1:2*n, i) + (h/2) * dy;        % Update the momentum halfway using the potential energy
        dy = f1(0.5 * (t(i+1) + t(i)), [y(1:n, i); pm]); % Compute the derivative (velocity) at the midpoint using updated momentum
        qm = y(1:n, i) + (h/2) * dy;            % Update the position halfway using the kinetic energy

        % Stormer-Verlet Step 2
        dy = f1(0.5 * (t(i+1) + t(i)), [qm; pm]); % Compute the derivative (velocity) at the midpoint using updated position and momentum
        y(1:n, i+1) = qm + (h/2) * dy;          % Update the position using the updated velocity
        dy = f2(t(i+1), y(:, i+1));             % Compute the derivative (momentum) at the next time using updated position
